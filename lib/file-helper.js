...
function replaceAssetPaths(html, opts) {
    ...
    return function (stylesheet) {
        ...
        var css = stylesheet.contents.toString().replace(/url\(['"]?([^'"\\)]+)['"]?\)/g, function (match, assetPath) {
            ...
            if (assetPath[0] == "/") {
                // HACK: restore missing "/" that got lost when generating vinyl, which uses a POSIX pathname
                const stylesheetUrl = stylesheet.path.replace(/^(https?\:\/)/, "$1/") // stylesheet.path url has only one /: "https:/strikingly..."
                const parsedUrl = url.parse(stylesheetUrl)
                return match.replace(assetPath, parsedUrl.protocol + "//" + parsedUrl.host + assetPath)
            }
...
function getPenthouseUrl(opts, file, port) {
    if (!opts.html && opts.src && isExternal(opts.src)) {
        return opts.src;
    }
...
function temp(opts) {
    return function (resp) {
        ...
        return tmp.fileAsync(_.assign(opts, {dir: "/tmp/", postfix: '.' + mime.extension(contentType)}))
            .then(getFirst)
            .then(function (path) {
                gc.addFile(path);
...
function getVinylPromise(opts) {
    ...
    if (opts.html) {
        ...
        return tmp.fileAsync({dir: "/tmp/", postfix: '.html'})
            .then(getFirst)
